@php
    $current_news_breaking = App\Models\NewsPost::where('status', 1)
        ->where('breaking_news', 1)
        ->latest()
        ->limit(1)
        ->get();
    
    $current_news = App\Models\NewsPost::where('status', 1)
        ->latest()
        ->skip(1)
        ->limit(12)
        ->get();
        ...
        <div class="sm:grid sm:grid-cols-5">
            ...
            <div class="sm:col-span-6">
                <div>
                    <!-- === Big article === -->
                    <div>
                        <a href="{{ url('news/details/' . $current_news_breaking[0]->id . '/' . $current_news_breaking[0]->news_title_slug) }}"
                            class="grid grid-cols-2 md:grid-cols-5 md:gap-5">
                            <!-- image -->
                            <div
                                ...
                                <img style="width: 1920px;" class="rounded-md"
                                    src="{{ asset($current_news_breaking[0]['image']) }}" alt="sample image" />
                            </div>
                            ...
                            <div class="self-end md:order-last justify-self-start md:self-start">
                                <p class="text-xs text-gray-500">
                                    @php
                                        get_times($current_news_breaking[0]->created_at);
                                    @endphp
                                </p>
                            ...
                            <div class="col-span-2">
                                ...
                                <h3 class="mt-4 md:mt-0 text-base font-bold">
                                    {{ $current_news_breaking[0]['news_title'] }}
                                </h3>
                                ...
                                <h4 class="mt-3 text-sm p-wrap">
                                    @php
                                        ...
                                        $string = strip_tags($current_news_breaking[0]['news_description']);
                                        echo $string;
                                    @endphp
                                </h4>
                            </div>
