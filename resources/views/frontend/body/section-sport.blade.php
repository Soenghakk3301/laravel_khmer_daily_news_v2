@php
    $sportnews = App\Models\NewsPost::where('status', 1)
        ->where('category_id', 4)
        ->latest()
        ->limit(4)
        ->get();
...
<section class="bg-black py-8 px-3 lg:mx-auto sm:px-10 md:px-24 lg:px-44 xl:px-50">
    <!-- herder -->
    ...
    <div class="grid grid-cols-2 gap-x-3 gap-y-8 sm:grid-cols-4">
        ...
        @foreach ($sportnews as $news)
            ...
            <a href="{{ url('news/details/' . $news->id . '/' . $news->news_title_slug) }}">
                ...
                <!-- time goes here -->
                <p class="mt-3 text-xs text-gray-400">{{ get_times($news->created_at) }}</p>

                <!-- title goes here -->
                <h3 class="mt-4 text-sm font-bold text-gray-200 sm:text-base">
                    {{ $news->news_title }}
                </h3>
            </a>
        @endforeach
